...
void gameRun(int a,int x,int y,int h[x][y],int v[y][x],int sq[y][y],int pc,int sc1[2],char m[],char name[][31],char n[],int p,int sc[],char t[])
{
    ...
    hConsole=GetStdHandle(STD_OUTPUT_HANDLE);
    int k=0;//counter
    char r[256],c[256];//r for row & c for column
    int r1=0,c1=0;
    int i=0;
...
void printing(int a,int x,int y,int h[x][y],int v[y][x],int sq[y][y],int pc,int sc[2],int sc1[2])
{
    ...
    {
        SetConsoleTextAttribute(hConsole,11);
        printf("To save the game and close press (s)\n");
        SetConsoleTextAttribute(hConsole,15);
    }
}
        ...
        if(n[1] == '\0')
        {
            ...
            case 'a':
                ...
                while (!feof(f))
                {
                    fscanf(f, "%[^,], %d",players1[count1].name,&players1[count1].score);
                    fscanf(f, "\n");
                    printf(" %d                    %s(%d)\n",count1+1,players1[count1].name,players1[count1].score);
                    count1++;
                }
                fclose(f);
            ...
            case 'b':
                ...
                while (!feof(f))
                {
                    fscanf(f, "%[^,], %d",players2[count2].name,&players2[count2].score);
                    fscanf(f, "\n");
                    printf(" %d                    %s(%d)\n",count2+1,players2[count2].name,players2[count2].score);
                    count2++;
                }
                fclose(f);
...
void MainMenu()
{
    char t[256];//type of line
    t[0]='\0';
    char m[256],n[256];//main menu variables to choose modes
    int p;//Counters
